<template>
    <nav
        :class="['bg-gray-800', 'fixed', 'top-0', 'left-0', 'text-white', isOpen ? 'w-64' : 'w-32', 'h-full', 'transition-width duration-300']">
            ...
            <ul v-if="isOpen" class="w-full">
                <li v-for="item in menuItems" :key="item.text" class="mb-2 flex flex-col items-start w-full">
                    ...
                    <div v-else class="flex items-center w-full">
                        ...
                        <router-link :to="item.link" class="block px-4 py-2 rounded hover:bg-gray-700 w-full">{{
...
const menuItems = [
    { text: 'Home', icon: HomeIcon, link: '/admin-dashboard' },
    { text: 'Users', icon: HomeIcon, link: '/list-user' },
    {
