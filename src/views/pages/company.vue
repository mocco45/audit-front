...
<template>
  ...
  <div class="container relative mx-auto p-4 min-h-screen">
    ...
    <nav class="absolute inset-x-0 bottom-0 flex items-center flex-column flex-wrap md:flex-row justify-between pt-4 bg-white dark:bg-gray-900" aria-label="Table navigation">
      <span class="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">
        Showing <span class="font-semibold text-gray-900 dark:text-white">{{ (page - 1) * itemsPerPage + 1 }}-{{ Math.min(page * itemsPerPage, totalCompanies) }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ totalCompanies }}</span>
      </span>
      <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
        <li>
          <button 
            :disabled="page === 1" 
            @click="fetchCompanies(page - 1)" 
