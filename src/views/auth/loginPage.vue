        ...
        <div class="w-1/2 p-8 flex flex-col justify-center">
          ...
          <form @submit.prevent="handleLogin" class="space-y-6">
            <div>
              ...
              <input type="email" @input="clearError('email')" v-model="email" id="email" class="rounded-full w-full border border-gray-400 px-3 py-2 shadow-lg focus:outline-none">
              <p v-if="errors.email" class="text-red-500 text-xs italic">{{ errors.email }}</p>
            </div>
            <div>
              <label for="password" class="block font-medium">Password</label>
              <input type="password" @input="clearError('password')" v-model="password" id="password" class="rounded-full w-full border border-gray-400 px-3 py-2 shadow-lg focus:outline-none">
